/**
 * SISTEMA VAGABOND GALLERY - MOSTRUÁRIO DE IMAGENS
 * 
 * Baseado no template das páginas One Piece e Naruto
 * Adaptado especificamente para o anime Vagabond
 */
class VagabondGallery {
    constructor() {
        // Lista de imagens do Vagabond
        this.images = [
            "https://i.ibb.co/zTZnQmTk/1.jpg",
            "https://i.ibb.co/BVbk3SjF/2.jpg",
            "https://i.ibb.co/VWN5kVM8/3.jpg"
        ];
        ...
        if (!this.galleryGrid) {
            console.log('Grid da gallery não encontrado - não inicializando VagabondGallery');
            return;
        }
     ...
     */
    init() {
        console.log('Inicializando VagabondGallery...');
        ...
        console.log('VagabondGallery inicializada com sucesso!');
    }
    ...
    createGalleryItems() {
        this.images.forEach((imgSrc, index) => {
            ...
            img.src = imgSrc;
            img.loading = "lazy";
            img.alt = `Vagabond - Placa decorativa ${index + 1}`;
    ...
    sendWhatsAppMessage(imageUrl, imageNumber) {
        const message = `🗾 Olá! Quero esta placa do Vagabond (#${imageNumber}): ${imageUrl}`;
...
/**
 * INICIALIZAÇÃO AUTOMÁTICA DO VAGABOND
 */
document.addEventListener('DOMContentLoaded', () => {
    if (document.getElementById('galleryGrid') && window.location.pathname.includes('vagabond.html')) {
        console.log('Página de gallery Vagabond detectada - inicializando...');
        const vagabondGallery = new VagabondGallery();
    }
});
