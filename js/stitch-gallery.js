/**
 * SISTEMA STITCH GALLERY - MOSTRUÁRIO DE IMAGENS
 * 
 * Baseado no template das páginas One Piece e Naruto
 * Adaptado especificamente para o Stitch
 */
class StitchGallery {
    constructor() {
        // Lista de imagens do Stitch
        this.images = [
        ...
        if (!this.galleryGrid) {
            console.log('Grid da gallery não encontrado - não inicializando StitchGallery');
            return;
        }
     ...
     */
    init() {
        console.log('Inicializando StitchGallery...');
        ...
        console.log('StitchGallery inicializada com sucesso!');
    }
    ...
    createGalleryItems() {
        this.images.forEach((imgSrc, index) => {
            ...
            img.src = imgSrc;
            img.loading = "lazy";
            img.alt = `Stitch - Placa decorativa ${index + 1}`;
    ...
    sendWhatsAppMessage(imageUrl, imageNumber) {
        const message = `👽 Olá! Quero esta placa do Stitch (#${imageNumber}): ${imageUrl}`;
...
/**
 * INICIALIZAÇÃO AUTOMÁTICA DO STITCH
 */
document.addEventListener('DOMContentLoaded', () => {
    if (document.getElementById('galleryGrid') && window.location.pathname.includes('stitch.html')) {
        console.log('Página de gallery Stitch detectada - inicializando...');
        const stitchGallery = new StitchGallery();
    }
});